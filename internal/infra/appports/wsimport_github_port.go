...
type wsImportGitHubPRView struct {
...
func (p *WSImportGitHubPort) SearchReviewPullRequests(ctx context.Context, scope wsimport.GitHubScope, maxResults int) ([]wsimport.GitHubPullRequest, error) {
	...
	args = append(args, "--json", "number,title,url,isDraft,repository")
	out, err := p.run(ctx, args...)
	if err != nil {
	...
	result := make([]wsimport.GitHubPullRequest, 0, len(items))
	for _, item := range items {
	...
	for i := range result {
		headRef, err := p.viewPullRequestHeadRef(ctx, result[i].Repository, result[i].Number)
		if err != nil {
			return nil, fmt.Errorf("resolve github pull request branch for %s#%d: %w", result[i].Repository, result[i].Number, err)
		}
		result[i].HeadRef = headRef
	}
	return result, nil
}
...
func (p *WSImportGitHubPort) viewPullRequestHeadRef(ctx context.Context, repo string, number int) (string, error) {
	out, err := p.run(ctx, "pr", "view", fmt.Sprintf("%d", number), "--repo", repo, "--json", "headRefName")
	if err != nil {
		return "", err
	}
	var view wsImportGitHubPRView
	if err := json.Unmarshal([]byte(out), &view); err != nil {
		return "", err
	}
	return strings.TrimSpace(view.HeadRefName), nil
}
